...
import { paymentVoucherBlocks } from './fees.pdf';
...
export async function renderStudentReceiptsPdf(ids: string[]): Promise<Buffer> {
  ...
  /*
   * The receipts themselves are the same paid-fee-voucher pages the challan and
   * the single-receipt print produce. Drawing them a second way here is what
   * made the statement and the receipt look like different documents.
   */
  const vouchers = await paymentVoucherBlocks(ids, {
    name: loaded.school?.name ?? 'School',
    address: loaded.school?.address ?? null,
    phone: loaded.school?.phone ?? null,
    email: null,
  ...
  const content: Content[] = [
    ...
    ...vouchers.map((v, i) =>
      oneStudent || i > 0
        ? ({ ...(v as unknown as Record<string, unknown>), pageBreak: 'before' } as unknown as Content)
        : v,
    ),
  ];
